         ...
         WHERE Line = 0
           ...
           AND ( "Public" = 1
                OR EXISTS (
                    SELECT 1
                      FROM Permissions
                     WHERE object = CONVERT( nvarchar(max), Id )
...
var saveObject = function(body) {
    ...
    tx.begin(err => {
        ...
        req.query('DELETE JsonObject WHERE Meta = @meta AND Id = @id AND UserId = @userId;', (err, result) => {
            ...
            var line;
            for (var i = 0; i < body.Text.length; i += 2048) {
            //     table.rows.add(
            //         id,
            //         i / 2048,
            //         body.Text.substr(i, 2048),
            //         body.UserId,
            //         body.Label,
            //         body.Meta,
            //         public
            //     );
                ...
                req.input('text' + line, sql.VarChar, body.Text.substr(i, 2048));
